 ...
 *
 * ORDER MATTERS: givens declared later in the same file are not in the implicit scope of earlier ones,
 * so the list is topologically sorted - every type appears after all case classes it references in its
 * fields (through Option/Vector/NonEmptyVector containers). Regenerate with the same dependency pass
