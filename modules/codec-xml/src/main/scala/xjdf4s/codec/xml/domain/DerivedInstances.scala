...
/**
 * GENERATED: `XmlElementCodec` givens for every case class without a hand-written codec. They live as
 * MEMBERS of this object and are exported from the `XmlElementCodec` companion (`export
 * DerivedInstances.given`), so they are found through the implicit scope of the type class from every
 * search context — including the implicit searches inside inline expansions, where lexical/package givens
 * whose right-hand side is another inline derivation are not found (the canonical `derives` companion-scope
 * strategy from the reference documentation).
 *
 * ORDER MATTERS inside the object: members declared later are not in the implicit scope of earlier ones,
 * so the list is topologically sorted by field dependencies. Regenerate with the same dependency pass
 * when the model grows. Excluded: hand codecs, attribute-only value types, the Audit family (dispatched
 * through AuditCodec) and the foreign fallback message carriers.
 */
