...
object XjdfIoInterpreters:
  ...
  final class Transport(
     ...
     *
     *  The bracket guarantees that the waiter registration is removed on completion, timeout, error, or
     *  cancellation, including cancellation while the injected sleep is still running.
     */
    private def awaitNewResponse(responseId: String): IO[Option[Response]] =
      Deferred[IO, Response].flatMap { deferred =>
        waits.update(_ + (responseId -> deferred)).bracket(_ => sleep(awaitTimeout) *> deferred.tryGet)(_ =>
          waits.update(_ - responseId)
        )
      }
