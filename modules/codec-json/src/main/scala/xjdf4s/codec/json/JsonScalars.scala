...
object JsonScalars:
  ...
  given Decoder[CmykColor] = Decoder.instance(cursor =>
    cursor.as[List[Double]].flatMap {
      case List(c, m, y, k) =>
        CmykColor.from(c, m, y, k).left.map(error => io.circe.DecodingFailure(error.toString, cursor.history))
  ...
  given Decoder[SrgbColor] = Decoder.instance(cursor =>
    cursor.as[List[Double]].flatMap {
      case List(r, g, b) =>
        SrgbColor.from(r, g, b).left.map(error => io.circe.DecodingFailure(error.toString, cursor.history))
      ...
      flat   <- points.foldLeft[Decoder.Result[Vector[Float]]](Right(Vector.empty)) {
        ...
        case (_, other) =>
          Left(io.circe.DecodingFailure(
            s"each TransferFunction point requires exactly two floats, got ${other.size}",
            cursor.history
          ))
      }
      function <-
        TransferFunction.from(flat).left.map(error => io.circe.DecodingFailure(error.toString, cursor.history))
    yield function,
  )
  ...
  given Decoder[LabColor] = Decoder.instance(cursor =>
    cursor.as[List[Double]].flatMap {
      case List(l, a, b) =>
        LabColor.from(l, a, b).left.map(error => io.circe.DecodingFailure(error.toString, cursor.history))
  ...
  def lexicalEnumCodec[A](values: Vector[A], lexical: A => String): (Encoder[A], Decoder[A]) =
    (
      ...
      Decoder.decodeString.emap(value =>
        values.find(candidate => lexical(candidate).equalsIgnoreCase(value)).toRight(
          s"'$value' is not a valid enum value"
        ),
      ),
    )
  ...
  given decoderVectorByte: Decoder[Vector[Byte]] =
    Decoder.decodeString.emap(value => Lexical.hexBinary(value).left.map(_.toString))
  ...
  given [A: Decoder]: Decoder[NonEmptyVector[A]] = Decoder.instance(cursor =>
    cursor
      .as[List[A]]
      .flatMap(values =>
        NonEmptyVector.from(values.toVector).left.map(_ =>
          io.circe.DecodingFailure("expected a non-empty array", cursor.history)
        )
      ),
  )
  ...
  given [A: Decoder]: Decoder[TwoOrMore[A]] = Decoder.instance(cursor =>
    cursor
      .as[List[A]]
      .flatMap(values =>
        TwoOrMore.from(values.toVector).left.map(_ =>
          io.circe.DecodingFailure("expected at least two elements", cursor.history)
        )
      ),
  )
  ...
  given [A: Decoder]: Decoder[AtMostTwo[A]] = Decoder.instance(cursor =>
    cursor
      .as[List[A]]
      .flatMap(values =>
        AtMostTwo.from(values.toVector).left.map(_ =>
          io.circe.DecodingFailure("expected at most two elements", cursor.history)
        )
      ),
  )
end JsonScalars
