...
object Main:
  ...
  private def demoAlignment(): Unit =
    ...
    val audit = Alignment.signalToAudit(
      FunctorForDemo.map(pulse)(identity).value
    )
  ...
  private def demoMatrix(): Unit =
    ...
    println(s"  identity == 1 0 0 1 0 0: ${Monoid[Matrix].empty == Matrix.identity}")

  /** Local helper to keep the demo free of extra imports. */
  private object FunctorForDemo:
    import cats.Functor
    import xjdf4s.model.Pulse
    def map[A, B](p: Pulse[A])(f: A => B): Pulse[B] = Functor[Pulse].map(p)(f)
