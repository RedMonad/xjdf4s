...
- **Slice 4 (in progress):** all 100 `SpecificResource` descendants grouped by general/prepress/press/postpress process
  domains. Forty foundational, postpress, and prepress resources plus the reusable `IdentificationField` graph are typed.
  See the generated status table in [resource-coverage.md](resource-coverage.md).
