...
| §6.73 | Table 6.148 | RunList | `RunList` | `?` | ✅ | ✅ | ❌ | ❌ | Implemented | `FileSpec?` → `Option[FileSpec]` (`minOccurs="0" maxOccurs="1"`; N-53), кардинальность закреплена regression- и XSD-oracle-тестами `RunListLaws`; New in XJDF 2.1: `@DocPages` → `RunList.docPages`; упражняется conformance-сьютом (`brochureJob`); `MetadataMap*` остаётся M1.6-6b |
...
| `MetadataMap*` (Table 8.46) не моделируется в срезе `IdentificationField` | элемент общий для `IdentificationField` (Table 8.31) и `RunList` (Table 6.148) и тянет за собой `Expr*` (Table 8.47), тип данных `XPath` и контекстные SHALL; предстартовая сверка B1 дополнительно выявила обязательность разрешения переменных `@ValueTemplate` через родителя / Table D.1 / ровно один `Expr`, которых прежнее описание среза не перечисляло | решение владельца 2026-08-16: M1.6-6b разделён на B1 (`XjdfXPath` + `Expr` + ADR-0013) и B2 (`MetadataMap`, оба wiring, полный набор контекстных SHALL); B1 реализован статически, поля-контейнеры остаются B2 | частично (M1.6-6b/B1, `[~]`) |
| `XjdfXPath` вместо Scala-имени `XPath` для типа данных Table A.1 | в `model.ValidationTypes` уже существует `model.XPath` — внутренний локатор ошибок; одинаковое имя при `model.*` + `prim.*` создало бы коллизии и риск смыслового смешения | wire/spec-имя остаётся `XPath`; новый номинальный тип называется `prim.XjdfXPath`, `Expr/@Path` принимает только его; `model.XPath` не меняется (ADR-0013, N-54) | реализовано статически (M1.6-6b/B1, `[~]`) |
| Appendix A задаёт `XPath` как `xsd:token`, а `schema.xsd` — restriction от `xs:string` | прямое расхождение prose/XSD: разные whitespace facets; release notes не содержат разъяснения | по §1.2 выбран Table A.1; `XjdfXPath.from` применяет XML whitespace collapse, расхождение закреплено тестом-оракулом; полная грамматика XPath отложена до M2 (ADR-0013, N-54) | реализовано статически (M1.6-6b/B1, `[~]`) |
