...
| `MetadataMap*` (Table 8.46) не моделируется в срезе `IdentificationField` | элемент общий для `IdentificationField` (Table 8.31) и `RunList` (Table 6.148) и тянет за собой `Expr*` (Table 8.47), тип данных `XPath` и два контекстно-зависимых SHALL («`Expr` SHALL NOT be specified in an `IdentificationField/MetadataMap`»; `MetadataMap/@Name` SHALL входить в родительский `@ValueTemplate`) — это второй семантический выбор, запрещённый §9.1 | поле в `IdentificationField` не заводится (заглушек нет); кардинальность `MetadataMap*` закреплена тестом сверки XSD в `IdentificationFieldLaws`; выделено в срез M1.6-6b, решение владельца от 2026-08-16 | отложено (M1.6-6b) |
