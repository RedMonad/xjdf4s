...
| §6.25 | Table 6.53 | CuttingParams | `CuttingParams` | `?` | ✅ | ✅ | ❌ | ❌ | Implemented | N-58: `FileSpec(CIP3)?` → `Option[FileSpec]`; regression/XSD-oracle coverage |
...
| §6.36 | Table 6.74 | FoldingParams | `FoldingParams` | `?` | ✅ | ✅ | ❌ | ❌ | Implemented | `FileSpec(CIP3)?` → `Option[FileSpec]` (N-58, regression/XSD oracle); `Crease*` (Table 8.17) моделируется (M1.6-2); `Cut*` — M3 |
...
| §8.19 | Table 8.22 | FileSpec | `FileSpec` | `*` | ✅ | ✅ | ❌ | ❌ | Implemented | N-51: локальный `FileSpec.law` запрещает смешение `@URL`/`@UID` с `@FileFormat`/`@FileTemplate` (`FILESPEC-LOCATION-GROUPS-CONFLICT`) и требует полную template-пару (`FILESPEC-TEMPLATE-INCOMPLETE`); `@URL`+`@UID` совместимы; полностью пустая location-группа разрешена только для pipe-контекста `ResourceSet/Dependent/@PipeID` (`FILESPEC-LOCATION-MISSING`, root validator); обходы: `ContentCheckIntent/ProofItem`, `CuttingParams`, `FoldingParams`, `Layout`, `Preview`, `RunList`; `Disposition?` сохраняет Table 8.23 law; `NetworkHeader*` → `Chain`; ID/IDREF нет. `@NPage` (New in 2.2) сохранён по prose/release notes вопреки отсутствию в XSD (ADR-0015/N-56). N-57: `@CheckSum?` → `Option[HexBinary]` по Table 8.22/XSD; лексика Appendix A (`0-9a-fA-F`, чётная длина, пустое значение допустимо, whitespace collapse), regression/XSD-oracle и round-trip; N-58 исправляет кардинальности четырёх контейнеров на `Option[FileSpec]` (`[x]`, верифицировано владельцем: 445/0) |
| §8.19.2 | Table 8.24 | NetworkHeader | `NetworkHeader` | `*` | ❌ | ✅ | ❌ | ❌ | Implemented | New in XJDF 2.1; `@Name: XjdfString` и `@Value: XjdfString` обязательны структурно; подэлементов и ID/IDREF нет; XSD `minOccurs="0" maxOccurs="unbounded"` под FileSpec |
...
| §6.52 | Table 6.95 | Layout | `Layout` | `?` | ✅ | ❌ | ❌ | ❌ | Implemented | New in XJDF 2.1: `@Anchor`, `@SheetLay` — моделируются; `FileSpec(ExternalImpositionTemplate)?` → `Option[FileSpec]` (N-58, regression/XSD oracle); `@ExpansionBox`, `Position/@PositionOrd` — не моделируются (M3) |
...
| §6.66 | Table 6.134 | Preview | `Preview` | `?` | ✅ | ❌ | ❌ | ❌ | Implemented | N-58: `FileSpec?` → `Option[FileSpec]`; regression/XSD-oracle coverage |
...
| Section | Table | Element/Attribute | Scala type | Cardinality | Validation | Domain tests | XML | JSON | Status | Notes |
|---------|-------|-------------------|------------|-------------|------------|--------------|-----|------|--------|-------|
| §A.1 | Table A.1 | hexBinary data type | `HexBinary` | 1 | ✅ | ✅ | ❌ | ❌ | Implemented | N-57: проверенный opaque-тип для `FileSpec/@CheckSum`; принимает пары цифр `0-9a-fA-F`, включая пустое значение, применяет фиксированный `xsd:hexBinary` whitespace collapse и сохраняет регистр; канонизация wire-регистра — M2 (ADR-0010) |
...
| `FileSpec/@NPage` есть в Table 8.22 и release notes XJDF 2.2, но отсутствует в XSD-объявлении FileSpec | прямое расхождение prose/release notes с `schema.xsd`; модель уже следовала prose и сохраняла `nPage: Option[Long]` без явного решения | ADR-0015/N-56: поле сохранено по §1.2; `FileSpecLaws` машинно фиксирует обе стороны; M2 обязан кодировать `@NPage`, а schema-conformance — учитывать известное исключение | `[x]` реализовано и верифицировано владельцем (N-51, 440/0) |
...
| Table 8.22 | `@NPage` (2.2) | моделируется (`FileSpec.nPage`); отсутствует в XSD, решение ADR-0015/N-56 |
| Table 8.24 | `NetworkHeader` (2.1) | моделируется как `FileSpec.networkHeaders: Chain[NetworkHeader]` (N-51) |
