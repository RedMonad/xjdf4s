...
- [x] `sbt "clean ; compile ; test"` зелёный (8 сюит, включая `XjmfChecksSuite` с 7 сценариями).
...
- **Прогон**: `sbt "clean ; compile ; test"` зелёный — все восемь сюит Passed; `XjmfChecksSuite` (7 сценариев)
  исполняет обе трассы и проверяет их совпадение.

**Итог этапа 06: транспорт XJMF исполним без сети — алгебра, машина каналов, корреляция refID, окна
замены и оба интерпретатора покрыты детерминированными тестами; точкой входа для HTTP-рантайма
(этап 07) служит `XjmfState.pending` (неотвеченные Reliable-сигналы) и сама алгебра `XjmfOp`.**
