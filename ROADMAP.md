...
3. **GADT-уточнение в `Part.get`/`PartBuilder.withKey`** (match type +
   singleton-ветки) — *снято*: сборка на Scala 3.8.4 подтвердила, что
   компилятор не уточняет абстрактный ключ в ветках матча при редукции
   `ValueOf[K]` («a match type could not be fully reduced»). Решение (в этом
   PR): типизированные поля и конструкторы `Part` + рантайм-доступ
   `Part.valueOf`/`PartBuilder.withValue` через тегованное `PartitionValue`;
   match type `ValueOf` остался как type-level свидетель. Типобезопасность
   домена не пострадала.
