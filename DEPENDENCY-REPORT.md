...
**Механизм E046 подтверждён по исходникам dotty (Typer.typedPackageDef, #25894/#26340):**
top-level `export` в пакете — то звено, которое замыкает цикл: lookup пакета (например, implicit-поиск
во время набора тела given'а) резолвит export-форвардеры `<src>$package`-класса того же файла, чьи
given'ы прямо сейчас типизируются, — цепочка возвращается в исходный поиск («Cyclic reference
involving val <import>`). Набор top-level export'ов фасада `JsonCodec` поэтому ЗАМОРОЖЕН на пяти
базовых объектах, доказанных зелёными сборками (Helpers/NodeCodecs/Scalars/MediaCodecs/
MessagingCodecs); кодеки аудитов/корня/спец-форм/ресурсов потребляются через явные
`import X.given` в местах использования. НЕ добавлять новые top-level export'ы без прогона сборки.
