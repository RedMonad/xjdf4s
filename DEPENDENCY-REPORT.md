...
## 🧑‍🔧 Ручная ревизия (добавлена после проверки)

**Цикл 1 (`FieldCodec ↔ Derived`) — реальный, намеренный, нарушением не является.**
Оба файла — ядро одного механизма деривации (`deriveOrSummon` из этапа 04): `Derived` собирает
per-field инстансы `FieldCodec`, а `FieldCodec.productCodec` при отсутствии готового кодека вложенного
типа деривирует его inline-вызовом `Derived.derived`. Разнести их по модулям нельзя без разрушения
inline-механизма; оба файла — в одном пакете и одном модуле (`codec-xml`). На уровне модулей циклов нет.

**Цикл 2 (шесть файлов `codec-json`) — реальный, ИСПРАВЛЕН.**
`JsonCodec` одновременно (а) содержал хелперы (`obj`/`memberList`/`opt`/`vec`/`fail`/`withSchema`...),
которые вызывают все кодек-объекты, и (б) реэкспортировал их givens. Хелперы вынесены в отдельный
`JsonHelpers.scala`; кодек-объекты ссылаются на `JsonHelpers`, а `JsonCodec` стал чисто фасадным
(только `export`, направленные наружу). После перегенерации отчёта цикл исчезнет.

**Цикл 3 (`Lexical ↔ XmlDecoders`) — ложное срабатывание, ИСПРАВЛЕНО.**
Единственное ребро `Lexical → XmlDecoders` порождалось scaladoc-ссылкой `[[XmlDecoders]]`; реальная
кодовая зависимость однонаправленная (`XmlDecoders → Lexical`, ссылка на тип `Lexical.Lex`). Ссылка
заменена обычным текстом. После перегенерации отчёта цикл исчезнет.

**Цикл 4 (43 файла `xjdf4s-model`) — файловый уровень одного модуля, нарушением не является.**
Все файлы — в одном модуле; взаимные ссылки типов внутри доменного модуля — нормальная практика.
Граф на уровне модулей ацикличен (`core ← model ← messaging ← protocol`; `dsl`, `codec-xml`,
`codec-json` — поверх). ADP соблюдён на уровне модулей.

**«God Object» `DerivedInstances` (Fan-Out 50) — артефакт генерации, не дефект.**
Файл содержит по одному given-кодеку на каждый деривированный case class (290 записей) — это
единственная разумная точка их размещения (требование не-inline givens из этапа 04). `Resource.scala`
(Fan-Out 39) — union-хаб `StandardSpecificResource`, как и в прошлой ревизии.

После перегенерации ожидается **2 цикла**: FieldCodec↔Derived (намеренный) и 43-файловый
внутримодульный.
